...
function EventCard({ event }: { event: CalendarEvent }) {
  ...
  return (
    <div className={`group p-3 rounded-md border ${typeStyles[event.type] || 'border-[#27272a] bg-[#0d0d0f]'}`}>
      ...
      {/* Title */}
      <h4 className="text-sm font-medium text-[#e8e8e8] mb-1">
        {event.title}
      </h4>
      
      {/* Time - always shown */}
      {event.startTime ? (
        <div className="flex items-center gap-1.5 text-xs text-[#a1a1a1] mb-2">
        ...
        <div className="text-xs text-[#525252] mb-2">All day</div>
      )}
      
      {/* Location */}
      {event.location && (
        <div className="flex items-center gap-2 text-xs text-[#a1a1a1] mb-2">
          <svg className="w-3.5 h-3.5 text-[#525252]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
